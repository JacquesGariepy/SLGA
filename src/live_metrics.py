...
class MetricsHistory:
    ...
    # Loss et PPL
    loss: deque = field(default_factory=lambda: deque(maxlen=50))
    ppl: deque = field(default_factory=lambda: deque(maxlen=50))
    val_loss: deque = field(default_factory=lambda: deque(maxlen=10))
    val_ppl: deque = field(default_factory=lambda: deque(maxlen=10))

    # Performance
    throughput: deque = field(default_factory=lambda: deque(maxlen=20))
    gpu_mem: deque = field(default_factory=lambda: deque(maxlen=20))

    # Training
    grad_norm: deque = field(default_factory=lambda: deque(maxlen=20))
    lr: deque = field(default_factory=lambda: deque(maxlen=20))
...
class LiveMetricsDisplay:
    ...
    def format_number(self, value: float, precision: int = 4, width: int = 8) -> str:
        ...
        if math.isnan(value) or math.isinf(value):
            return f"{Colors.BRIGHT_RED}{str(value):>{width}}{Colors.RESET}"
    ...
    def detect_anomalies(self, metrics: Dict) -> List[str]:
        ...
        if metrics.get('gpu_memory_used_pct', 0) > 0.95:
            warnings.append(f"{Symbols.WARNING} GPU memory critical: {metrics['gpu_memory_used_pct']*100:.0f}%")
    ...
    def _render(
        ...
        # === HEADER ===
        print(f"{Colors.BOLD}{Colors.BRIGHT_CYAN}{Symbols.ROCKET} SLGA Training Live Metrics{Colors.RESET}")
        print(f"{Colors.DIM}Step {step:,} / {self.max_steps:,} ({progress*100:.1f}%) │ Elapsed: {self.format_time(elapsed_total)} │ ETA: {self.format_time(eta_seconds)}{Colors.RESET}")
        ...
        print(f"  Loss:       {loss_color}{self.format_number(loss, precision=4)}{Colors.RESET} {loss_trend}  │  Best: {Colors.GREEN}{self.best_loss:.4f}{Colors.RESET}")
        print(f"  Perplexity: {loss_color}{self.format_number(ppl, precision=2)}{Colors.RESET} {ppl_trend}  │  Best: {Colors.GREEN}{self.best_ppl:.2f}{Colors.RESET}")
        print(f"  Learn Rate: {Colors.CYAN}{lr:.6e}{Colors.RESET}  │  Grad Norm: {Colors.CYAN}{metrics.get('grad_norm', 0.0):.4f}{Colors.RESET}")
        print()
        ...
        if 'val_loss' in metrics:
            ...
            print(f"  Val Loss:   {self.format_number(val_loss, precision=4)}  │  Val PPL: {self.format_number(val_ppl, precision=2)}")
            print(f"  Train/Val Gap: {gap_color}{gap:+.4f}{Colors.RESET}  │  Best Val: {Colors.GREEN}{self.best_val_loss:.4f}{Colors.RESET}")
            print()
        ...
        print(f"  Throughput:  {Colors.BRIGHT_GREEN}{tokens_per_sec:>8,.0f}{Colors.RESET} tok/s  │  {Colors.GREEN}{samples_per_sec:>6,.1f}{Colors.RESET} samples/s")
        ...
        print(f"  GPU Memory:  {mem_color}{gpu_mem_gb:>6.2f}{Colors.RESET} / {gpu_mem_total_gb:.1f} GB  ({self.format_percentage(gpu_mem_pct)})")
        print()
        ...
        if 'num_landmarks' in metrics and metrics['num_landmarks'] > 0:
            ...
            print(f"  Selected:     {Colors.CYAN}{num_lm:>4}{Colors.RESET}  │  Global Weight: {self.format_percentage(global_weight)}")
            print(f"  Spacing Loss: {self.format_number(spacing_loss, precision=4)}  │  Sparsity Loss: {self.format_number(sparsity_loss, precision=4)}")
            print()

        # === MINI CHART (tendance loss) ===
        if len(self.history.loss) > 10 and not self.compact:
            print(f"{Colors.BOLD}{Symbols.CHART} Loss Trend (last {len(self.history.loss)} steps){Colors.RESET}")
            print(f"{'─' * self.width}")

            chart_lines = self.make_mini_chart(list(self.history.loss), height=3, width=min(50, self.width - 10))
