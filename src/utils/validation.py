...
class ConfigValidator:
    ...
    def validate_slga_config(config: Dict[str, Any]) -> List[ValidationResult]:
        ...
        for key in ["embed_dim", "num_heads", "local_window", "max_seq_len", "global_k"]:
            if config[key] <= 0:
                results.append(
                    ValidationResult(
                        passed=False, message=f"{key}={config[key]} must be > 0", severity="error"
                    )
                )
        ...
        if not results:
            results.append(
                ValidationResult(
                    passed=True, message="All SLGA config parameters valid", severity="info"
                )
            )
    ...
    def validate_training_config(config: Dict[str, Any]) -> List[ValidationResult]:
        ...
        if not results:
            results.append(
                ValidationResult(
                    passed=True, message="All training config parameters valid", severity="info"
                )
            )
...
class RuntimeValidator:
    ...
    @staticmethod
    def check_gradients(
        model: nn.Module, warn_threshold: float = 1e-7, error_threshold: float = 100.0
    ) -> ValidationResult:
        ...
        if not grad_norms:
            return ValidationResult(
                passed=False, message="No gradients found (possible detach issue)", severity="error"
            )
    ...
    @staticmethod
    def check_loss(
        loss: torch.Tensor, step: int, max_loss: Optional[float] = None
    ) -> ValidationResult:
        ...
        if torch.isnan(loss):
            return ValidationResult(
                passed=False, message=f"NaN loss at step {step}", severity="error"
            )

        if torch.isinf(loss):
            return ValidationResult(
                passed=False, message=f"Inf loss at step {step}", severity="error"
            )
        ...
        return ValidationResult(passed=True, message=f"Loss valid: {loss_val:.4f}", severity="info")

    @staticmethod
    def check_landmarks(
        landmark_indices: Optional[torch.Tensor], seq_len: int, min_unique_ratio: float = 0.9
    ) -> ValidationResult:
        ...
        if landmark_indices is None:
            return ValidationResult(
                passed=True, message="No landmarks (using heuristic)", severity="info"
            )
    ...
    def check_model_outputs(
        outputs: torch.Tensor, vocab_size: int, check_logits: bool = True
    ) -> ValidationResult:
        ...
        if torch.isnan(outputs).any():
            return ValidationResult(
                passed=False, message="NaN values in model outputs", severity="error"
            )

        if torch.isinf(outputs).any():
            return ValidationResult(
                passed=False, message="Inf values in model outputs", severity="error"
            )
        ...
        if not check_logits:
            ...
            if not torch.allclose(probs_sum, torch.ones_like(probs_sum), atol=1e-3):
                return ValidationResult(
                    passed=False, message="Probabilities don't sum to 1", severity="warning"
                )

        return ValidationResult(
            passed=True, message=f"Model outputs valid (shape: {outputs.shape})", severity="info"
        )


def print_validation_results(results: List[ValidationResult], verbose: bool = True) -> None:
