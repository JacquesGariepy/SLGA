...
class CleanedDataset:
    ...
    def __init__(self, dataset: Any, text_key: str = "text") -> None:
        """
        Args:
    ...
    def __getitem__(self, idx: int) -> Union[Dict[str, Any], str, Any]:
        ...
        # Sinon, retourner tel quel (Any type)
        return example
    ...
    def clean_text(text: Any) -> Any:
        ...
        Args:
            text: Texte à nettoyer (str expected, but Any accepted)

        Returns:
            text_clean: Texte nettoyé (ou original si pas une string)
        """
    ...
    def select(self, indices: Any) -> CleanedDataset:
...
def test_cleaner() -> None:
    ...
    assert (
        result1 == expected1
    ), f"Failed: Unicode replacement. Got '{result1}', expected '{expected1}'"
