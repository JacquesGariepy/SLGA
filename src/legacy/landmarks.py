...
class LearnableLandmarkSelector(nn.Module):
    ...
    def _gumbel_topk(
        ...
        if torch.isnan(gumbel_noise).any() or torch.isinf(gumbel_noise).any():
            print("⚠️ NaN/Inf detected in Gumbel noise after fix, using fallback")
        ...
        if torch.isnan(soft_scores).any():
            print("❌ NaN detected in soft_scores after Gumbel softmax!")
            print(
                f"   Perturbed scores - min: {perturbed_scores.min().item()}, max: {perturbed_scores.max().item()}"
            )
    ...
    def forward(
        ...
        if torch.isnan(selection_scores).any():
            print("❌ NaN detected in selection_scores after softmax!")
            print(
                f"   Scores before softmax - min: {scores.min().item()}, max: {scores.max().item()}"
            )
            print("   Clamp applied ([-20, 20]) but NaN persists - possible overflow")
            # Fallback: uniform distribution
...
def landmark_spacing_loss(
    ...
    if selection_scores is not None:
        ...
        positions = torch.arange(
            seq_len, device=selection_scores.device, dtype=selection_scores.dtype
        )  # (L,)
        ...
        normalized_scores = masked_scores / (
            masked_scores.sum(dim=1, keepdim=True) + 1e-8
        )  # (B, L)
        ...
        segment_centers = (
            torch.arange(G, device=selection_scores.device, dtype=selection_scores.dtype)
            * segment_size
            + segment_size / 2
        )  # (G,)
        ...
        segment_indices = (
            torch.arange(seq_len, device=selection_scores.device).float() / segment_size
        ).long()
        ...
        segment_weights = torch.zeros(
            B, G, device=selection_scores.device, dtype=selection_scores.dtype
        )
...
def landmark_sparsity_loss(
    selection_scores: torch.Tensor, num_landmarks: int, lambda_reg: float = 0.001
) -> torch.Tensor:
    """
