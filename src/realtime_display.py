...
class RealtimeTrainingDisplay:
    ...
    def __init__(
        ...
        self.step_times = deque(maxlen=20)  # Pour smooth throughput
    ...
    def _print_header(self):
        ...
        print(f"{Colors.BOLD}{Colors.BRIGHT_CYAN}🚀 SLGA Training - Real-Time Display{Colors.RESET}")
        print(f"{Colors.BOLD}{Colors.BRIGHT_CYAN}{'='*self.width}{Colors.RESET}")
        print()
    ...
    def update_live(
        ...
        # GPU percentage
        gpu_pct = self.current_gpu_mem / self.current_gpu_total if self.current_gpu_total > 0 else 0
    ...
    def print_detailed(
        ...
        # Training params
        print(
            f"   LR: {Colors.CYAN}{lr:.6e}{Colors.RESET} │ "
            f"Grad: {Colors.CYAN}{grad_norm:.4f}{Colors.RESET} │ "
            f"SeqLen: {Colors.CYAN}{seq_len:,}{Colors.RESET} │ "
            f"GW: {Colors.CYAN}{global_weight:.2f}{Colors.RESET}"
        )
    ...
    def print_validation(self, step: int, val_loss: float, val_ppl: float):
